...
const LAUNCHPAD_STEPS = [
  'Create collection', 'Review', 'Market admission',
  'Cold market', 'Traction', 'Hot market',
]

function PhoneFrame({ children }) {
  return (
    <div style={{ position: 'relative', width: '260px', margin: '0 auto' }}>
      <div style={{ position: 'relative', borderRadius: '42px', background: 'linear-gradient(180deg,#1a1b1d,#0a0a0a)', padding: '10px', boxShadow: '0 40px 80px rgba(0,0,0,0.6),0 0 0 1px rgba(255,255,255,0.04)' }}>
    ...
    <div style={{ display:'flex', flexDirection:'column', justifyContent:'space-between', height:'100%', padding:'44px 20px 28px', background:'#0A0A0A' }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'12px', flex:1, justifyContent:'center', margin:'16px 0' }}>
        ...
        <div style={{ display:'flex', justifyContent:'center' }}>
          <div style={{ width:'28px', height:'28px', borderRadius:'50%', border:`1px solid ${stage==='matching'?'#B8BEC8':'rgba(255,255,255,0.2)'}`, display:'flex', alignItems:'center', justifyContent:'center' }}>
...
function FiatScene() {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setCurrIdx(i => { const next=(i+1)%CURRENCIES.length; if(next===0) setOnRamp(o=>!o); return next })
    }, 1800)
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', padding:'44px 20px 28px', background:'#0A0A0A' }}>
      <div style={{ display:'flex', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'20px', padding:'4px', marginBottom:'24px', position:'relative' }}>
        <div style={{ position:'absolute', top:'4px', bottom:'4px', width:'calc(50% - 4px)', background:'#ECEEF1', borderRadius:'16px', transition:'transform 0.3s ease', transform:onRamp?'translateX(0)':'translateX(100%)', left:'4px' }} />
        <span style={{ flex:1, textAlign:'center', fontFamily:'IBM Plex Mono,monospace', fontSize:'10px', letterSpacing:'0.14em', textTransform:'uppercase', color:onRamp?'#000':'#4B5563', padding:'8px', position:'relative', zIndex:1 }}>On-ramp</span>
        <span style={{ flex:1, textAlign:'center', fontFamily:'IBM Plex Mono,monospace', fontSize:'10px', letterSpacing:'0.14em', textTransform:'uppercase', color:!onRamp?'#000':'#4B5563', padding:'8px', position:'relative', zIndex:1 }}>Off-ramp</span>
      </div>
      ...
      <div style={{ display:'flex', justifyContent:'center', gap:'6px', marginTop:'16px' }}>
        {CURRENCIES.map((c,i) => <span key={c} style={{ width:'4px', height:'4px', borderRadius:'50%', background:i===currIdx?'#B8BEC8':'rgba(255,255,255,0.1)', display:'inline-block', transition:'background 0.3s' }} />)}
      </div>
    </div>
  )
}
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', padding:'44px 20px 28px', background:'#0A0A0A' }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', flex:1, justifyContent:'center' }}>
        {LAUNCHPAD_STEPS.map((step,i) => {
          ...
          return (
            <div key={step} style={{ display:'flex', alignItems:'flex-start', gap:'12px', paddingBottom:i<LAUNCHPAD_STEPS.length-1?'14px':'0' }}>
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center' }}>
                ...
                {i<LAUNCHPAD_STEPS.length-1 && <div style={{ width:'1px', height:'14px', background:done?'rgba(184,190,200,0.5)':'rgba(255,255,255,0.06)', marginTop:'4px' }} />}
              </div>
    ...
    <div style={{ background:'#050505', color:'#ECEEF1', minHeight:'100vh', fontFamily:'Manrope,sans-serif' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .l-desk { display: flex !important; }
        .l-mob  { display: none  !important; }
        @media (max-width: 768px) {
          .l-desk { display: none  !important; }
          .l-mob  { display: flex  !important; }
        }
      `}</style>
      ...
      <header style={{ position:'fixed', top:0, left:0, right:0, zIndex:100, height:'64px', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 24px', borderBottom:'1px solid rgba(255,255,255,0.06)', background:'rgba(5,5,5,0.85)', backdropFilter:'blur(20px)' }}>
        ...
        {/* Desktop nav */}
        <nav className="l-desk" style={{ alignItems:'center', gap:'28px' }}>
          {navLinks.map(l => (
          ...
          <button onClick={()=>setShowWaitlist(true)} style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.15em', color:'#4B5563', background:'none', border:'none', cursor:'pointer' }}
            onMouseEnter={e=>e.target.style.color='#ECEEF1'} onMouseLeave={e=>e.target.style.color='#4B5563'}>
            Waitlist
          </button>
        </nav>
        ...
        <Link href={DAPP_URL} className="l-desk" style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.14em', color:'#000', background:'#ECEEF1', padding:'9px 22px', borderRadius:'20px', fontWeight:'500' }}>
          Launch App
        </Link>

        {/* Mobile hamburger */}
        <button className="l-mob" onClick={()=>setMobileMenu(true)} style={{ alignItems:'center', justifyContent:'center', width:'38px', height:'38px', background:'none', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'8px', color:'#ECEEF1', fontSize:'18px', cursor:'pointer', lineHeight:1 }}>
          ☰
        </button>
      </header>
      ...
      {mobileMenu && (
        <>
          <div onClick={()=>setMobileMenu(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', zIndex:200, backdropFilter:'blur(4px)' }} />
          <div style={{ position:'fixed', top:0, right:0, bottom:0, width:'280px', background:'#0A0A0A', borderLeft:'1px solid rgba(255,255,255,0.08)', zIndex:300, display:'flex', flexDirection:'column', padding:'20px 16px' }}>
            <div style={{ display:'flex', justifyContent:'flex-end', marginBottom:'20px' }}>
              <button onClick={()=>setMobileMenu(false)} style={{ background:'none', border:'none', color:'#4B5563', fontSize:'22px', cursor:'pointer' }}>✕</button>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:'4px', flex:1 }}>
              {navLinks.map(l => (
                <a key={l.label} href={l.href} target={l.external?'_blank':undefined} rel={l.external?'noopener noreferrer':undefined}
                  onClick={()=>setMobileMenu(false)}
                  style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'12px', textTransform:'uppercase', letterSpacing:'0.15em', color:'#9CA3AF', padding:'14px 12px', borderRadius:'10px', display:'block', transition:'all 0.2s' }}
                  onMouseEnter={e=>{e.target.style.color='#ECEEF1';e.target.style.background='rgba(255,255,255,0.04)'}}
                  onMouseLeave={e=>{e.target.style.color='#9CA3AF';e.target.style.background='transparent'}}>
                  {l.label}
                </a>
              ))}
              <button onClick={()=>{setMobileMenu(false);setShowWaitlist(true)}}
                style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'12px', textTransform:'uppercase', letterSpacing:'0.15em', color:'#9CA3AF', padding:'14px 12px', borderRadius:'10px', background:'none', border:'none', cursor:'pointer', textAlign:'left', width:'100%' }}>
                Waitlist
              </button>
            </div>
            <div style={{ borderTop:'1px solid rgba(255,255,255,0.06)', paddingTop:'16px' }}>
              <Link href={DAPP_URL} onClick={()=>setMobileMenu(false)}
                style={{ display:'block', width:'100%', padding:'13px', background:'#ECEEF1', borderRadius:'20px', fontFamily:'IBM Plex Mono,monospace', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.14em', color:'#000', fontWeight:'500', textAlign:'center' }}>
                Launch App
              </Link>
            </div>
          </div>
      ...
      {/* ── Hero ─────────────────────────────────────────────── */}
      <section style={{ minHeight:'92vh', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'100px 40px 60px', position:'relative', overflow:'hidden', maxWidth:'1200px', margin:'0 auto', gap:'60px', flexWrap:'wrap' }}>
        ...
        <div style={{ maxWidth:'520px', position:'relative', zIndex:1 }}>
          <p style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.2em', color:'#4B5563', marginBottom:'20px' }}>Flibber · Slotting Mechanism</p>
          <h1 style={{ fontSize:'clamp(40px,6vw,68px)', fontWeight:'500', lineHeight:'1.05', letterSpacing:'-0.02em', marginBottom:'24px' }}>
            ...
            <br /><span style={{ color:'#ECEEF1' }}>Keep value.</span>
          </h1>
          ...
          <div style={{ display:'flex', gap:'16px', flexWrap:'wrap' }}>
            <Link href={DAPP_URL} style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.14em', color:'#000', background:'#ECEEF1', padding:'14px 28px', borderRadius:'20px', fontWeight:'500' }}>Launch App</Link>
            <button onClick={()=>setShowWaitlist(true)} style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.14em', color:'#6B7280', background:'none', border:'1px solid rgba(107,114,128,0.3)', padding:'14px 28px', borderRadius:'20px', cursor:'pointer' }}>Join Waitlist</button>
          </div>
        </div>
        <div style={{ flexShrink:0 }} className="l-desk">
      ...
      <section style={{ maxWidth:'1100px', margin:'0 auto', padding:'80px 40px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:'56px', flexWrap:'wrap', gap:'16px' }}>
          <h2 style={{ fontSize:'clamp(26px,3.5vw,38px)', fontWeight:'500', letterSpacing:'-0.02em', color:'#ECEEF1', maxWidth:'380px' }}>Three utilities, one mechanism.</h2>
          <div style={{ display:'flex', gap:'8px' }}>
            <button onClick={()=>setActiveUtil(a=>Math.max(a-1,0))} disabled={activeUtil===0} style={{ width:'36px', height:'36px', borderRadius:'50%', border:'1px solid rgba(255,255,255,0.1)', background:'none', color:'#B8BEC8', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', opacity:activeUtil===0?0.3:1, fontSize:'18px' }}>‹</button>
            <button onClick={()=>setActiveUtil(a=>Math.min(a+1,UTILITIES.length-1))} disabled={activeUtil===UTILITIES.length-1} style={{ width:'36px', height:'36px', borderRadius:'50%', border:'1px solid rgba(255,255,255,0.1)', background:'none', color:'#B8BEC8', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', opacity:activeUtil===UTILITIES.length-1?0.3:1, fontSize:'18px' }}>›</button>
          </div>
        </div>
      ...
      <section style={{ maxWidth:'700px', margin:'0 auto', padding:'80px 40px', textAlign:'center' }}>
        <p style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.2em', color:'#4B5563', marginBottom:'16px' }}>The Slotting Mechanism</p>
      ...
      <section id="testnet" style={{ maxWidth:'700px', margin:'0 auto', padding:'80px 40px 120px', textAlign:'center' }}>
        <p style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.2em', color:'#4B5563', marginBottom:'16px' }}>Flibber Testnet</p>
        ...
        <div style={{ display:'flex', gap:'16px', justifyContent:'center', flexWrap:'wrap' }}>
          <Link href={DAPP_URL} style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.14em', color:'#000', background:'#ECEEF1', padding:'14px 32px', borderRadius:'20px', fontWeight:'500' }}>Launch App</Link>
          <button onClick={()=>setShowWaitlist(true)} style={{ fontFamily:'IBM Plex Mono,monospace', fontSize:'11px', textTransform:'uppercase', letterSpacing:'0.14em', color:'#6B7280', background:'none', border:'1px solid rgba(107,114,128,0.3)', padding:'14px 32px', borderRadius:'20px', cursor:'pointer' }}>Join Waitlist</button>
        </div>
      </section>
        ...
        <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'60px 40px', display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:'40px' }}>
          <div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              {SOCIALS.map(s => (
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" style={{ fontSize:'14px', color:'#4B5563', transition:'color 0.2s' }}
                  onMouseEnter={e=>e.target.style.color='#ECEEF1'} onMouseLeave={e=>e.target.style.color='#4B5563'}>{s.label}</a>
              ))}
            </div>
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap:'10px', alignItems:'flex-end' }}>
            {[{label:'Documentation',href:'/docs'},{label:'Contact',href:'mailto:flibberfdn@gmail.com'},{label:'Privacy Policy',href:'/privacy'},{label:'Terms of Service',href:'/terms'}].map(p => (
              <a key={p.label} href={p.href} style={{ fontSize:'14px', color:'#4B5563', transition:'color 0.2s' }}
                onMouseEnter={e=>e.target.style.color='#ECEEF1'} onMouseLeave={e=>e.target.style.color='#4B5563'}>{p.label}</a>
            ))}
          </div>
        </div>
